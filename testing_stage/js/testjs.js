$(function(){
	...
	var Sort = function(data) {
		...
		this.imgSrc = ko.computed(function(){
			var filename;
			if (this.display()) {
				filename = "img/" + this.status() + "_large.png"
			} else {
		...
		this.hover = ko.computed(function(){
			var hoverstring = this.display() ? 'Mask ' : 'Reveal ';
			hoverstring += this.status();
			return hoverstring;
		}, this);
	};
...
*/
      ...
      return 0;
		...
		for (var incident in incidentDataMain) {
		  ...
		  for (var i = 0; i < incidentData[incident].information.length; i++) {
		    ...
		    if (victims.indexOf(place.victim) < 0) {
		      victims.push(place.victim);
		    }
		  }
		}
	    ...
	    self.filterList = function() {
	      ...
	      for (var filter in self.filters()) {
	        if (self.filters()[filter].display()) {
	          visibleEvents.push(self.filters()[filter].victim());
	        }
	      }
	    ...
	    for (var j = 0; j < victims.length; j++) {
	      victim = new Sort({ 'victim': victims[j], 'display': true });
	      victim.display.subscribe(self.filterList);
	      self.filters.push(victim);
	    }
	};
	...
	var isIncidentDisplayed = function(incident, filterincidents) {
	    ...
	    for (var victim in isincident) {
	      if (filterincidents.indexOf(isincident[victim]) >= 0) {
	        displayed = true;
	        break;
	      }
	    }
